...
def check_rf_grow(n_data, n_large, n_draws,
    ...
    for i, max_depth in depth_iter:
        for j in np.arange(n_draws):
            ...
            data_tune, y_tune = smooth_base.generate_data(large_n=n_large)
            y_tune = y_tune + 100
            # test
            ...
            smooth_ew_oracle_nc, smooth_ew_oracle_nc_last, _, c5 = smooth_base.smooth(
                                model_fit,
                                X_trained = data,
                                y_trained = y.ravel(),
                                X_tune = data_tune,
                                ...
                                verbose = False,
                                all_trees = True)
            smooth_ew_oracle_c, smooth_ew_oracle_c_last, _, c6 = smooth_base.smooth(
                                model_fit,
                                X_trained = data,
                                y_trained = y.ravel(),
                                X_tune = data_tune,
    ...
    return score_mat, [c1, c2, c3, c4, c5, c6]
...
if create_figs:
    ...
    if select == 1:
        ...
        data_vis.to_csv("images/tree1_reg_all_microsoft_o.csv")

        save_as_pdf_pages([reg_vis1 + labs(title = "1 tree, reg") +\
                           theme(figure_size = (8,6))],
                          filename = "images/tree1_reg_all_microsoft_o.pdf")
    ...
    if select == 10:
        ...
        save_as_pdf_pages([reg_vis10 + labs(title = "10 tree, reg") +\
                           theme(figure_size = (8,6))],
                         filename = "images/tree10_reg_all_microsoft_o.pdf")
    ...
    if select == 300:
        ...
        data_vis300.to_csv("images/tree300_reg_all_microsoft_o.csv")

        save_as_pdf_pages([reg_vis300 + labs(title = "300 tree, reg") +\
                           theme(figure_size = (8,6))],
                         filename = "images/tree300_reg_all_microsoft_o.pdf")
